...
% 1.2
% plot_circle(hall_color, 3, 'r');
plot_circle(hall_color, 3, [255, 0, 0], './report/asserts/1_2_');

% plot_chess(hall_color, 10);
plot_chess(hall_color, 10, './report/asserts/1_2_');
...
disp("Time of matlab dct2, square: " + toc);
tic;
for i = 1:100000
    c_patch00_ = dct_2(patch00);
end
disp("Time of my dct2, square: " + toc);
tic;
for i = 1:100000
    c_patch00 = dct2(patch00, [6, 10]);
end
disp("Time of matlab dct2, rectangle: " + toc);
tic;
for i = 1:100000
    c_patch00_ = dct_2(patch00, [6, 10]);
end
disp("Time of my dct2, rectangle: " + toc);
...
freqz(b, a);
% save_or_wait(gcf);
save_or_wait(gcf, false, './report/asserts/2_5_freqz');
...
% 2.12
[DC_stream, AC_stream, rows, cols] = jpeg_encode(hall_gray, QTAB, DCTAB, ACTAB);
disp("Compression ratio origin: " + (8 * rows * cols) / (length(DC_stream) + length(AC_stream)));
hall_gray_decode = jpeg_decode(DC_stream, AC_stream, rows, cols, QTAB, DCTAB, ACTAB);
disp('PSNR origin: ' + string(cal_PSNR(hall_gray, hall_gray_decode)));
QTAB_ = QTAB / 2;
[DC_stream, AC_stream, rows, cols] = jpeg_encode(hall_gray, QTAB_, DCTAB, ACTAB);
disp("Compression ratio half: " + (8 * rows * cols) / (length(DC_stream) + length(AC_stream)));
hall_gray_decode_ = jpeg_decode(DC_stream, AC_stream, rows, cols, QTAB_, DCTAB, ACTAB);
disp('PSNR half: ' + string(cal_PSNR(hall_gray, hall_gray_decode_)));
subplots(1, 2, {hall_gray_decode, hall_gray_decode_}, {'Original Decode', 'Half Quantization Decode'});
% save_or_wait(gcf);
...
eesast = imread('./resource/eesast-group-photo-2023.jpg');
eesast_dark = eesast - 50;
eesast_ratio = [0.525, 0.735, 0.87];
for L = 3:5
    v = train(faces, L);
    face_recognition(eesast_dark, 16, 16, 4, 4, L, v, eesast_ratio(L - 2));
    % save_or_wait(gcf);
    save_or_wait(gcf, true, ['./report/asserts/4_4_eesast_L' num2str(L)]);
end